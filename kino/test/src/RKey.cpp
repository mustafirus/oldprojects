...
void RKey::Get(char* str)
{
	FKey* pfk = *pQTable;
	if(pfk)
		str += sprintf(str, "%s;%s",pfk->pTable->name().c_str(), pfk->name);
	else
		str += sprintf(str, "%s;%s",((Table*)*pQTable)->name().c_str(),
...
bool RKey::CanSet(char*& str)
{
	...
	if(s1 != true)
		return false;
	name = ((FKey*)*pQTable) ? ((FKey*)*pQTable)->pTable->name().c_str() :
	((Table*)*pQTable)->name().c_str();
	if(strcmp(str, name))
		return false;
