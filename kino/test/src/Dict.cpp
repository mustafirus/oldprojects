...
Table* Dict::GetTableByName(const char* name)
{
	...
	{
		if(!strcmp(pTables[i]->name().c_str(), name))
			return pTables[i];
	}
...
{
	return GetForm(pt->name().c_str(), pt->mPKey.name, n, type);
}
...
char* Dict::GetForm(const char* key, int n, int type /*= 0*/)
{
	...
	char* str = GetForm(pt->name().c_str(), pt->mPKey.name, n, type);
	ASSERT(str);
	return str;
}
...
char* Dict::GetForm(RKey* prk, int n, int type /*= 0*/)
{
	...
	char* str = NULL;
	if(pfk)
		str = GetForm(pfk->pTable->name().c_str(), pfk->name, n, type);
	if(!str)
	{
		Table* pt = *((QTable*)*prk);
		str = GetForm(pt->name().c_str(), pt->mPKey.name, n, type);
	}
	return str;
}
